...
export class ChatbotComponent implements OnInit {
  ...
  clickedOptions: Set<number> = new Set<number>();
  isProcessing: boolean = false
  step: any
  services: any = ["Doorstep Delivery", "Book an Appointment", "Emergency Service"]
  selectedService: any = ''
  userInput: any = ''
  responseStructure: any[] = [{ heading: "", getmsg: "", options: [], notemsg: "", input: "", emergency: "", patientName: "", doctorName: "", department: "", date: "", timeslot: "", rensendOtp: '' }]
  departments: any[] = []
  ...
  @Input() closeChatBot: boolean = true;
  ...
  ngOnInit(): void {
    ...
    const month = String(this.minDate.getMonth() + 1).padStart(2, '0'); // Months are zero-based
  ...
  async loadDepartments(): Promise<void> {
    try {
      this.serviceChoosen = "Appointment"
  ...
  async sendingAppointmentMail(): Promise<void> {
    ...
    if (userconfirm === "1" || userconfirm === "Yes, proceed with the booking." || userconfirm === "yeah" || userconfirm === "yes") {
      ...
      const appointmentData = {
        ...
        email: '',
        doctorName: this.selectedDoctor.name,
        department: this.selectedDepartment.name, // Convert array to string if necessary, // Assuming `speciality` is the department
        date: this.selectedDate,
      ...
      this.chatbotService.createAppointment(appointmentData).toPromise();
  ...
  appointmentThankMsg(): void {
    this.userInput = ''
    const newEntry = {
      heading: `Thank you, ${this.userInfo.firstName} ${this.userInfo.lastName}! We have received your ${this.serviceChoosen} service request.`,
      getmsg: " Our team will get back to you shortly.",
      options: "",
      notemsg: "",
      input: "Yes, proceed with the booking",
      emergency: "",
      patientName: "",
      doctorName: "",
      department: "",
    ...
    this.userReceivedSms();
    this.responseStructure.push(newEntry)
    this.userInput = ''
  }

  userReceivedSms(): void {
    const details = {
      patientName: `${this.userInfo.firstName} ${this.userInfo.lastName}`,
      // otp: this.otp,
    ...
    this.chatbotService.whatsApp(details).toPromise()
  }
  ...
  async getDoorStepService(): Promise<void> {
    ...
    const newEntry = {
      ...
      notemsg: "(Note : Doorstep services are available from 7:00 AM to 7:00 PM)",
      input: this.userInput,
      emergency: "",
      patientName: "",
      doctorName: "",
  ...
  async getNameDoorStep(): Promise<void> {
    if (this.userInput === "1" || this.userInput === "Pharmacy" || this.userInput === "pharmacy") {
      ...
      this.serviceChoosen = "Blood Sample Collection"
      const newEntry = {
  ...
  handleOptionClick(option: string, optionIndex: number): void {
    ...
    // if (this.clickedOptions.has(optionIndex)) {
    //   return; // If already clicked, do nothing
    // }
    if (option.startsWith("Yes") || option.startsWith("No") || option.endsWith("Location")) {
      this.disableOptions = true;
    }
    this.isProcessing = true
    // this.clickedOptions.add(optionIndex);
